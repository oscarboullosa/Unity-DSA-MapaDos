...
public class NinjaBoyMovement : MonoBehaviour
{
    ...
    void Update()
    {
        if (!colision)
        {
            ...
            animator.SetTrigger("NinjaBoyRun");
        }
        else
        {
            animator.ResetTrigger("NinjaBoyRun");
        }
        if (playRedJump)
        {
            animator.SetTrigger("NinjaBoyJump");
            playRedJump = false;
        }
    }
    ...
    private IEnumerator OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (SceneManager.GetActiveScene().name == "Game4Win")
        {
            if (collision.gameObject.CompareTag("Juego1") || collision.gameObject.CompareTag("Juego2") || collision.gameObject.CompareTag("Juego3") || collision.gameObject.CompareTag("Juego5"))
            {
                ...
                AsyncOperation asyncLoad = SceneManager.LoadSceneAsync("FirstScene5");
